...
from sine_exmodel import HierachicalClassifier
from dataloader import DataIter
import json

def evaluate(model,loss_function,batch_generator,cuda=None,id2word=None,output_file=None):
    if not cuda is None:
        model.cuda(cuda)
    model.eval()
    ...
    with torch.no_grad():
        for batch in batch_generator:
            ...
            predicted_target,attention_weight,classifier_input,_,senatt_arr,kld_loss,recon_loss= model(data_var_list,tfidf_var_list,length,flag="gen_ex",id2word=id2word,output_file=output_file)
...
#total_loss/(batch_i+1),acc,true_label_array,predicted_label_array,returned_document_list,total_kld_loss/(batch_i+1)
                ...
                filename = "bestmodel_{}_{}".format(dataset,topic_learning)
...
def main():
    ...
    for i, param in enumerate(params_search):
        ...
        id2word = {vocab[key]:key for key in vocab.keys()}
        output_file = "explanations_{}.txt".format(args.dataset)
        f = open(output_file,"w")
        f.close()
        evaluate(model,loss_function,test_batch,cuda=args.cuda,id2word=id2word,output_file=output_file)
