...
for bin in [0,1,2,3,4]:
    f0 = pd.read_csv("/home/hanq1yanwarwick/SINE/eraser_metric/output_logits/SINE/{}/{}/prediction_results_original_999bin_SelfExplanation.csv".format(dataset,variants),index_col="idx")
    f1 = pd.read_csv("/home/hanq1yanwarwick/SINE/eraser_metric/output_logits/SINE/{}/{}/prediction_results_{}_{}bin_SelfExplanation.csv".format(dataset,variants,metric,bin),index_col="idx")
        ...
        if True:
            ...
            if float(combine_df.iloc[i]["base_pre"]) == 1.0:
                ...
                # diff.append(base_logits-logits)
                    diff.append(combine_df.iloc[i]["base_1logits"]-combine_df.iloc[i]["1logits"])
                ...
                # diff.append(base_logits-logits)
                diff.append(combine_df.iloc[i]["base_0logits"]-combine_df.iloc[i]["0logits"])
            elif float(combine_df.iloc[i]["base_pre"]) == 2:
                diff.append(combine_df.iloc[i]["base_2logits"]-combine_df.iloc[i]["2logits"])
            elif float(combine_df.iloc[i]["base_pre"]) == 3:
                diff.append(combine_df.iloc[i]["base_3logits"]-combine_df.iloc[i]["3logits"])
            elif float(combine_df.iloc[i]["base_pre"]) == 4:
                diff.append(combine_df.iloc[i]["base_4logits"]-combine_df.iloc[i]["4logits"])
    # if metric == "cset":
