...
# sys.path.insert(0, '/home/hanq1yanwarwick/eraserbenchmark/')
...
def partial_match_score(truth: List[Rationale], pred: List[Rationale], thresholds: List[float]) -> List[Dict[str, Any]]:
    ...
    num_classifications = {k:len(v) for k,v in pred_to_rat.items()} #get the number of predicted rats for each instance
    num_truth = {k:len(v) for k,v in ann_to_rat.items()}#get the number of GT rats for each instance
    ious = defaultdict(dict)
    for k in set(ann_to_rat.keys()) | set(pred_to_rat.keys()):#the union of the two sets
        for p in pred_to_rat.get(k, []): #get the predicted rats for doc k
            best_iou = 0.0
            for t in ann_to_rat.get(k, []):
                ...
                if iou > best_iou:
                    best_iou = iou
            ious[k][p] = best_iou#record the overlap for each rat(p) in doc k 
    scores = []
    for threshold in thresholds:
        threshold_tps = dict()
        for k, vs in ious.items():
            threshold_tps[k] = sum(int(x >= threshold) for x in vs.values())# for each doc, calculate the number of overthreshold
...
def score_hard_rationale_predictions(truth: List[Rationale], pred: List[Rationale]) -> Dict[str, Dict[str, float]]:
    ...
    truth = set(truth)
    pred = set(pred)
    micro_prec = len(truth & pred) / len(pred) #exact overlap
...
def main():
    ...
    parser.add_argument('--data_dir', dest='data_dir', default="/home/hanq1yanwarwick/SINE/input_data/eraser_movie/movies",required=False, help='Which directory contains a {train,val,test}.jsonl file?')
    parser.add_argument('--split', dest='split', required=False, default= "test",help='Which of {train,val,test} are we scoring on?')
    parser.add_argument('--strict', dest='strict', required=False, action='store_true', default=False, help='Do we perform strict scoring?')
    parser.add_argument('--results', dest='results', default="/home/hanq1yanwarwick/SINE/explain_metric/SINE_test_decoder_02bin_att_imdbweight.json",required=False, help="""Results File
    ...
    """)
    parser.add_argument('--iou_thresholds', dest='iou_thresholds', required=False, nargs='+', type=float, default=[0.1,0.3,0.5], help='''Thresholds for IOU scoring.
    ...
    ''')
    parser.add_argument('--score_file', dest='score_file', required=False, default="/home/hanq1yanwarwick/SINE/explain_metric/SINE_test_score_02bin_att_imdbweight.jsonl", help='Where to write results?')
    ...
    if _has_hard_predictions(results):
        ...
        if args.iou_thresholds is not None:
            ...
            scores['iou_scores'] = iou_scores#only calculate the number of overthreshold
        # NER style scoring
        rationale_level_prf = score_hard_rationale_predictions(truth, pred)
        scores['rationale_prf'] = rationale_level_prf #like accuracy
        token_level_truth = list(chain.from_iterable(rat.to_token_level() for rat in truth))#transform the span to token-level 
